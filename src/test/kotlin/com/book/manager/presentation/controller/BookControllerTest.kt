...
internal class BookControllerTest {
    ...
    fun `getList is success`() {
        ...
        val mockMvc = MockMvcBuilders.standaloneSetup(bookController).build() // 対象のControllerのオブジェクトを設定
        val resultResponse = mockMvc.perform(get("/book/list")).andExpect(status().isOk).andReturn().response
        // perform: HTTPメソッドの指定，対処Controllerのパスの設定
        // andExpect: 期待されるHTTPステータスの設定
        // andReturn: 結果の返却
        // response: 結果からレスポンスのオブジェクトを取得
        val result = resultResponse.getContentAsString(StandardCharsets.UTF_8) // getContentAsString関数でJSON文字例に変換
