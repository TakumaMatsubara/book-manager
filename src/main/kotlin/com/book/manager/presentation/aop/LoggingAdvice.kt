...
@Component
class LoggingAdvice {
    @Before("execution(* com.book.manager.presentation.controller..*.*(..))") // 前処理の実装 引数について：(戻り値 package名.class名.関数名(引数の型))
    ...
    @After("execution(* com.book.manager.presentation.controller..*.*(..))") // 後処理の実装
